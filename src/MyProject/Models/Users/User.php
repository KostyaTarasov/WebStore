...
class User extends ActiveRecordEntity // получаем все возможности, что есть у сущности Article
{
    /** @var string */
    protected $nickname;
    ...
    public static function signUp(array $userData)
    {
        ...
        if (empty($userData['nickname'])) { // если пустое значение
            throw new InvalidArgumentException('Не передан nickname'); // вызываем исключение
        }
        ...
        if (static::findOneByColumn('nickname', $userData['nickname']) !== null) { // Где в параметрах имя nickname и значение введённое пользователем
        ...
        // Мы не будем передавать после того как вошли на сайт в cookie ни пароль, ни его хеш. 
        // Мы будем использовать только этот токен, который у каждого пользователя будет свой и он никак не будет связан с паролем – так безопаснее.
        $user->save(); // В конце метода мы сохраняем этого нового пользователя в базу (внутри через вызов метода insert
    ...
    public static function login(array $loginData): User
    {
        if (empty($loginData['email'])) {
            throw new InvalidArgumentException('Не передан email'); // вызываем исключение
        }
